...
function PricingCard({ pricingTable, saleFlags, coachingId, planType, duration, isPopular, index }) {
    const price = pricingTable[coachingId]?.[planType]?.[duration.months] || 0;
    const [hovered, setHovered] = useState(false);
    // Sale flag is set per exact cell (coaching type + plan type + duration)
    // from the admin Pricing page — independent of `popular`, so a card can
    // be Most Popular AND On Sale at the same time.
    const isOnSale = saleFlags.includes(`${coachingId}:${planType}:${duration.months}`);
    ...
    return (
        ...
        >
            ...
            {/* Flashing SALE badge, top-right corner, whenever this duration is marked on_sale */}
            {isOnSale && (
                <motion.span
                ...
                <AnimatePresence mode="wait">
                    {isBasicSelected ? (
                        ...
                        <motion.div key={`${activeTab}-${planType}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                            ...
                            {durations.map((dur, i) => (
                                <PricingCard key={dur.months} pricingTable={pricingTable} saleFlags={saleFlags} coachingId={activeTab} planType={planType} duration={dur} isPopular={!!dur.popular} index={i} />
                            ))}
                        </motion.div>
                    )}
                </AnimatePresence>
