...
export function Navbar() {
    ...
    const mobileMenu = isMobileMenuOpen ? (
        <div style={{
            position: "fixed", inset: 0, backgroundColor: "#0a0a0a", zIndex: 999999,
            display: "flex", flexDirection: "column", overflowY: "auto",
            transform: isVisible ? "translateX(0)" : "translateX(100%)",
            opacity: isVisible ? 1 : 0,
            transition: "transform 350ms cubic-bezier(0.32, 0.72, 0, 1), opacity 300ms ease",
        }}>
            <div style={{
            ...
            <div style={{ display: "flex", flexDirection: "column", padding: "16px", gap: "4px" }}>
                {navItems.map((item, index) => (
                    <button key={item.name} onClick={() => handleNavClick(item)} style={{
                        all: "unset", display: "flex", width: "100%", alignItems: "center",
                        justifyContent: "space-between", padding: "14px 16px", borderRadius: "12px",
            ...
            >
                <div className="container mx-auto flex items-center justify-between px-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={openMenu} className="p-2 lg:hidden text-white">
