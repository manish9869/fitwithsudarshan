// src/pages/admin/diet/DietWorkoutTemplateList.jsx
// List page for diet_workout_templates — same pattern as DietTemplateList.jsx.
import { useState, useEffect, useMemo } from 'react';
import { useNavigate } from 'react-router-dom';
import { Plus, Search, Loader2, Edit2, Trash2, Repeat2 } from 'lucide-react';
import { listCmsRows, deleteCmsRow } from '../content/cmsApi';
import { useToast } from '../ToastProvider';
import PaginationBar from '../PaginationBar';

export default function DietWorkoutTemplateList() {
    ...
    const load = async () => {
        setLoading(true);
        try { setRows(await listCmsRows('diet_workout_templates')); }
    ...
    const filtered = useMemo(() => {
        ...
        return rows.filter((r) => [r.name, r.description].some((v) => String(v || '').toLowerCase().includes(q)));
    }, [rows, search]);
    ...
    const handleDelete = async () => {
        try {
            await deleteCmsRow('diet_workout_templates', deleteTarget.id);
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
                <div>
                    <h1 className="text-xl font-black text-white">Workout Templates</h1>
                    <p className="text-xs text-white/35 mt-0.5">{rows.length} {rows.length === 1 ? 'template' : 'templates'} — exercise rotations used alongside Diet Plan Templates</p>
                </div>
                <button onClick={() => navigate('/admin/workout-templates/new')}
                                    ...
                                    <tr key={row.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                                        <td className="px-4 py-3">
                                            <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'rgba(231,23,99,0.1)' }}>
                                                <Repeat2 className="w-3.5 h-3.5" style={{ color: '#e71763' }} />
                                            </div>
                                        </td>
                                        <td className="px-4 py-3">
                                        ...
                                        <td className="px-4 py-3 hidden md:table-cell">
                                            <span className="text-xs text-white/50">{(row.exercise_days || []).length} day{(row.exercise_days || []).length === 1 ? '' : 's'}</span>
                                        </td>
                                        <td className="px-4 py-3">
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-1 justify-end">
                                                <button onClick={() => navigate(`/admin/workout-templates/${row.id}`)} title="Edit" className="w-8 h-8 rounded-lg flex items-center justify-center text-white/40 hover:text-white hover:bg-white/8"><Edit2 className="w-3.5 h-3.5" /></button>
