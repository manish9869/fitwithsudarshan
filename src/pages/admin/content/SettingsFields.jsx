...
// ── Tag/chip input for short string lists (e.g. plan features) — type a
// value and press Enter (or just click away) to turn it into a pill.
// Backspace on an empty input deletes the last pill. Far quicker to use
// than a stacked list of "one text box per item" or a comma-separated field.
export function ChipInput({ label, value, onChange, placeholder, hint }) {
    const items = Array.isArray(value) ? value : [];
    ...
    return (
        <div>
            {label && <label className={labelCls}>{label}</label>}
            <div className="flex flex-wrap items-center gap-2 px-3 py-2.5 rounded-lg" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
                {items.map((item, i) => (
                    <span key={i} className="flex items-center gap-1.5 pl-3 pr-1.5 py-1 rounded-full text-xs font-semibold text-white whitespace-nowrap"
                        style={{ background: 'rgba(231,23,99,0.15)', border: '1px solid rgba(231,23,99,0.3)' }}>
                        {item}
                        <button type="button" onClick={() => removeAt(i)} className="w-4 h-4 rounded-full flex items-center justify-center text-white/60 hover:text-white hover:bg-white/15">
