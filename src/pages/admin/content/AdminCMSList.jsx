...
import { Plus, Edit2, Trash2, X, Loader2, AlertCircle, ImagePlus, ImageOff, Search, ImageIcon } from 'lucide-react';
import { CMS_CONFIGS } from './cmsConfigs';
import { listCmsRows, createCmsRow, updateCmsRow, deleteCmsRow, uploadImage } from './cmsApi';
...
// Stores just the resulting CDN URL as the field value (same shape as the
// old plain-text URL field), so nothing downstream needs to change.
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <button
                        ...
                        disabled={uploading}
                        className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold text-white disabled:opacity-60"
                    ...
                    {value && (
                        <button
                            type="button"
                            onClick={() => onChange('')}
                            className="ml-2 text-xs text-white/30 hover:text-red-400"
                        >
                            Remove
                        </button>
                    )}
...
export default function AdminCMSList() {
    ...
    useEffect(() => { load(); setSearch(''); }, [table]);
