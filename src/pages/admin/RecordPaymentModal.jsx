...
export default function RecordPaymentModal({ enrollment, onClose, onSaved }) {
    ...
    const buildReference = () => {
        const parts = [];
        if (orderRef.trim()) parts.push(`Order: ${orderRef.trim()}`);
        if (paymentRef.trim()) parts.push(`Payment: ${paymentRef.trim()}`);
        return parts.length ? parts.join(' · ') : null;
    };

    const handleSave = async () => {
        if (isFullyPaid) return; // hard block — no balance to collect against
        setError('');
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                ...
                <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                    <div>
                        ...
                        <p className="text-[11px] text-white/35 mt-0.5">
                            Balance due: <span style={{ color: isFullyPaid ? '#34d399' : '#fbbf24' }} className="font-bold">
                ...
                <div className="p-5 space-y-4">
                    {isFullyPaid ? (
                        <div className="flex items-start gap-2.5 px-3 py-3 rounded-xl text-xs"
                            style={{ background: 'rgba(52,211,153,0.06)', border: '1px solid rgba(52,211,153,0.2)', color: 'rgba(255,255,255,0.6)' }}>
                            <Ban className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" style={{ color: '#34d399' }} />
                            <span>
                                This enrollment is already fully paid — there's no balance to record a payment
                                against. Close this and adjust the total price first if a new charge is owed.
                            </span>
                        </div>
                    ) : (
                        <>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="text-[11px] font-bold text-white/45 uppercase tracking-widest mb-1.5 block">
                                        Order / Batch Ref
                                    </label>
                                    <input value={orderRef} onChange={(e) => setOrderRef(e.target.value)} placeholder="Optional"
                                        className="w-full rounded-xl px-3.5 py-2.5 text-sm text-white outline-none"
                                        style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }} />
                                </div>
                                <div>
                            ...
                            <p className="text-[10px] text-white/25 -mt-1.5 leading-relaxed">
                                If a client pays in several installments under the same order, use the same
                                Order/Batch Ref each time but a different Payment ID/UTR per installment.
                            </p>
                    ...
                    <div className="flex gap-3 pt-1">
                        <button onClick={onClose} className="flex-1 py-2.5 rounded-xl text-sm text-white/40" style={{ border: '1px solid rgba(255,255,255,0.08)' }}>
