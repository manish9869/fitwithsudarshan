/**
 * PaymentFailed.jsx — Enhanced premium payment failure page
 */
...
import {
    ...
    Hash, Zap, WifiOff, CreditCard, Lock, DollarSign,
} from 'lucide-react';
        ...
        <div className="relative flex items-center justify-center w-32 h-32 mx-auto mb-8">
            {[1, 2].map((i) => (
                <motion.div
                    ...
                    style={{ inset: `-${i * 14}px`, border: `1px solid rgba(239,68,68,${0.28 - i * 0.1})` }}
...
function getErrorInfo(message = '') {
    ...
    if (lower.includes('declined') || lower.includes('insufficient') || lower.includes('failed'))
        return { short: 'Transaction Declined', hint: 'Your bank declined the transaction. This may be due to insufficient funds, a card limit, or a security block from your bank.' };
    if (lower.includes('network') || lower.includes('connection'))
        return { short: 'Network Issue', hint: 'Your connection dropped during checkout. No charge was made. Please ensure a stable connection and retry.' };
    if (lower.includes('gateway'))
        return { short: 'Gateway Error', hint: 'The payment gateway encountered a temporary issue. This usually resolves in a moment — please try again.' };
    ...
    return { short: 'Payment Unsuccessful', hint: 'The payment could not be completed. No charge has been made to your account. You can safely try again.' };
}

// ─── Reason card ──────────────────────────────────────────────────────────────
function ReasonCard({ icon: Icon, label, hint, color }) {
    return (
        <div className="rounded-2xl p-4 transition-all hover:scale-[1.02]"
            ...
            <p className="text-[11px] text-white/28 leading-relaxed">{hint}</p>
        </div>
    );
}
...
export default function PaymentFailed() {
    ...
    return (
        ...
        >
            ...
            {/* ── Navbar ── */}
            <div className="relative z-20 sticky top-0 backdrop-blur-2xl"
                ...
                <div className="container mx-auto px-6 py-3 flex items-center justify-between max-w-5xl">
                    <div className="flex items-center gap-2.5">
                        ...
                        />
                        <span className="text-sm font-bold text-white/85">FitWith<span style={{ color: '#e71763' }}>Sudarshan</span></span>
                    </div>
            ...
            <div className="relative z-10 container mx-auto px-4 py-14 max-w-2xl">
                ...
                >
                    ...
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[11px] font-black uppercase tracking-[0.16em] mb-5"
                        style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.18)', color: '#f87171' }}>
                        <AlertTriangle className="w-3 h-3" />
                        {errorInfo.short}
                    </div>
                    ...
                    <p className="text-white/35 max-w-xs mx-auto text-sm leading-relaxed">
                        {errorInfo.hint}
                    </p>
                </motion.div>
                ...
                >
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-[10px] font-black uppercase tracking-widest text-white/25 mb-1.5">Error Details</p>
                            <p className="text-sm text-white/55 leading-relaxed">{raw}</p>
                        </div>
                    </div>
                    ...
                    {partialEnrollment?.enrollmentId && (
                        <div className="flex items-center gap-2 mt-4 pt-4"
                            ...
                            <Hash className="w-3.5 h-3.5 text-white/18" />
                            <span className="text-xs text-white/22">Order ref:</span>
                            <span className="text-xs font-mono text-white/38">{partialEnrollment.enrollmentId}</span>
                        </div>
                    )}
                </motion.div>
                ...
                >
                    ...
                    >
                        ...
                        <ChevronRight className="w-4 h-4 ml-auto text-white/22" />
                    </motion.a>
                ...
                >
                    <p className="text-[11px] font-black uppercase tracking-[0.18em] text-white/20 mb-5 text-center">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <ReasonCard icon={WifiOff} label="Network Issue" hint="A stable connection is required. Switch networks and retry." color="251,191,36" />
                        <ReasonCard icon={DollarSign} label="Insufficient Funds" hint="Check your available balance before retrying." color="96,165,250" />
                        <ReasonCard icon={Lock} label="Card Limit Reached" hint="Use a different card, net banking, or UPI instead." color="52,211,153" />
                    </div>
                </motion.div>
                ...
                >
                    <p className="text-[10px] font-black uppercase tracking-[0.18em] text-white/22 mb-4 text-center">Direct Support</p>
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                        <a href="mailto:Fitwithsudarshanofficial@gmail.com"
                            ...
                            <Mail className="w-3.5 h-3.5" style={{ color: '#e71763' }} />
                            Fitwithsudarshanofficial@gmail.com
                        </a>
                        <div className="hidden sm:block w-px h-4" style={{ background: 'rgba(255,255,255,0.08)' }} />
                        <a href="https://wa.me/919619708124"
                            className="flex items-center gap-2 text-xs text-white/35 hover:text-white/55 transition-colors">
                            <Phone className="w-3.5 h-3.5" style={{ color: '#e71763' }} />
                            +91 96197 08124
                        </a>
                    </div>
                </motion.div>
            ...
            {/* Footer */}
            <div className="relative z-10 border-t mt-12" style={{ borderColor: 'rgba(255,255,255,0.04)' }}>
                <div className="container mx-auto px-6 py-6 max-w-5xl flex flex-col sm:flex-row items-center justify-between gap-3">
                    <span className="text-xs text-white/18">© 2024 FitWithSudarshan — RECODE™ Program</span>
                    <span className="text-xs text-white/14">Payments powered by Razorpay · PCI-DSS Compliant</span>
                </div>
            </div>
        </motion.div>
    );
